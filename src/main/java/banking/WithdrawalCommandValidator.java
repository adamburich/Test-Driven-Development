...
public class WithdrawalCommandValidator {
    ...
    public boolean validate(Command c) {
        ...
        withdrawing_from_real_account = withdrawal_target_is_int && withdrawal_amount_is_double && (bank.getAccount(Integer.parseInt(account_id)) != null);
        Account account = bank.getAccount(Integer.parseInt(account_id));
        withdrawing_from_nonempty_account = account.getBalance() > 0;
